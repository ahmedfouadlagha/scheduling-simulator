...
from core.SchedulerEvent import SchedulerBeginScheduleEvent, \
...
class SchedulerInfo(object):
    ...
    def instantiate(self, model):
        ...
        Args:
            - `model`: The :class:`Model <core.Model.Model>` object \
...
class Scheduler(object):
    ...
    the :class:`jobs <core.Job.Job>` and the :class:`processors
    ...
    simulation:
        ...
        <core.Processor.Processor.resched>` method is required.
    ...
    def __init__(self, sim, scheduler_info, **kwargs):
        ...
        - `sim`: :class:`Model <core.Model>` instance.
        - `scheduler_info`: A :class:`SchedulerInfo` representing the \
            scheduler.

        Attributes:

        - **sim**: :class:`Model <core.Model.Model>` instance. \
            Useful to get current time with ``sim.now_ms()`` (in ms) or \
            ``sim.now()`` (in cycles).
        - **processors**: List of :class:`processors \
            <core.Processor.Processor>` handled by this scheduler.
        - **task_list**: List of :class:`tasks <core.Task.GenericTask>` \
    ...
    def on_activate(self, job):
        ...
        Args:
            - `job`: The activated :class:`job <core.Job.Job>`.
        """
        pass
    ...
    def on_terminated(self, job):
        ...
        Args:
            - `job`: The :class:`job <core.Job.Job>` that terminates .
        """
        pass
    ...
    def schedule(self, cpu):
        ...
        Args:
            - `cpu`: The :class:`processor <core.Processor.Processor>` \
    ...
    def add_task(self, task):
        ...
        Args:
            - `task`: The :class:`task <core.Task.GenericTask>` to add.
        """
    ...
    def add_processor(self, cpu):
        ...
        Args:
            ...
            <core.Processor.Processor>` to add.
        """
...
def get_schedulers():
    ...
    if getattr(sys, 'frozen', False):
        ...
        for name in importer.toc:
            if name.startswith('schedulers.'):
                modules.append(name)

    # Normal case:
    else:
        package = importlib.import_module('schedulers')
